...
public class Sec extends NormalTrigonometric {
	...
	public String evaluate() {
		try { 
			...
			if(super.getAngleType() == AngleNotation.DEGREES) {
				return String.valueOf(Math.toDegrees(1/Math.cos(angle)));
			}
			//Else, check if the angle type is Radians and return the result in radian form if it is
				...
				return String.valueOf(Math.toRadians(1/Math.cos(angle)));
			}
			else {
